...
import { validatorID, gardID, oracleID, openFeeID, closeFeeID } from "./ids";
import { reserve, treasury, cdpGen } from "./contracts";
import { accountInfo, getParams, sendTxn, getWallet, getAppByID, signGroup } from "../wallets/wallets";
...
export let currentPrice = .816; // XXX: This should be kept close to the actual price - it is updated on initialization though
let currentFee = {
...
export async function getPrice() {
	...
	const currentPriceJSON = await $.getJSON("https://storage.googleapis.com/algo-pricing-data-2022/latest_pricing.json");
...
export function calcDevFees(amount, close=false) {
	...
	return Math.floor((amount*fee*(10**currentDecimals)) / (1000*currentBigPrice)) + 10000
}
...
export async function updateCDPs(address) {
	...
	let webcalls = 0;
	for (const x of Array(MAXID-MINID).fill().map((_, i) => i + MINID)) {
		if ( !accountCDPs || !(accountCDPs.hasOwnProperty(x)) || (accountCDPs[x]['checked'] + (mins_to_refresh * 60 * 1000) < Date.now())) {
...
async function findOpenID(address) {
	...
	for (const x of Array(MAXID-MINID).fill().map((_, i) => i + MINID)) {
		if ( !accountCDPs || !(accountCDPs.hasOwnProperty(x)) || (accountCDPs[x]['state'] == 'closed')) {
...
export async function openCDP(openingALGOs, openingGARD) {
	...
	if (ratio < MINRATIO) {
		return { 
			alert: true, 
			text: "Ratio needs to be above " + MINRATIO + "%.\n" +
					"Your ratio is: " +  calcRatio(openingMicroALGOs, openingGARD, true) 
			} 
	}
	...
	if (307000 + openingMicroALGOs + 100000*(info["assets"].length + 1) + devFees > info["amount"])
	{
		return {
				alert: true,
				text: "Depositing this much collateral will put you below your minimum balance.\n" +
				"Your Maximum deposit is: " +  (info["amount"] - devFees - 307000 - 100000*(info["assets"].length + 1))/1000000 + " Algos"
				}
	} 
...
export async function mint(accountID, newGARD) {
	...
	let lsigReserve = algosdk.makeLogicSig(reserve.logic, [algosdk.encodeUint64(2)]);
...
export async function addCollateral(accountID, newAlgos) {
	...
	if (newAlgos + 100000*(info["assets"].length + 1) > info["amount"])
	{
		return {
			alert: true,
			text: "Depositing this much collateral will put you below your minimum balance.\n" +
			"Your Maximum deposit is: " +  (newAlgos + 100000*(info["assets"].length + 1))/1000000 + " Algos"
		}
	}
	let cdp = cdpGen(info.address, accountID);
...
function updateCDP(address, id, newCollateral, newDebt, state = 'open', commitment=0) {
...
export async function commitCDP(account_id, amount) {
	...
	const stringVal = 'af/gov1:j{"com":' + parseInt(amount*1000000).toString() + '}';
...
export async function liquidate(account_id, owner_address, microDebt, microPremium) {
	...
	if (gard_bal == null || gard_bal < (microDebt + to_user + liquid_fee)){
		return {
			alert: true,
			text: "Insufficient GARD for transaction. Balance: " + ((gard_bal/1000000).toFixed(2)).toString() + '\n' + 
			"Required: " + (((microDebt + to_user + liquid_fee)/1000000).toFixed(2)).toString()
			}
	}
