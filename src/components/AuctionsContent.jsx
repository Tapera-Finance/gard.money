...
import { getChainData, getCurrentAlgoUsd, getCurrentUnix } from '../prices/prices'
      ...
      result.push(
        {
          ...
          cost: debt / 1000000,
          premium: Math.max((Math.floor(debt*23/20)-Math.floor(debt*(curr_time-auction_start)/24000) - debt) / 1000000, 0)
        })
      count += 1
...
export default function AuctionsContent() {
  ...
  let defaulted = addresses.map((value, idx) => {
    ...
    return {
      ...
      marketDiscount: (100*(1 - ((cdp_data[idx].cost + cdp_data[idx].premium)/(cdp_data[idx].amount*curr_price)))).toFixed(1),
        ...
        {selected === OPTIONS.LIVE_AUCTIONS ? (
          <TransactionSummary
            ...
            transactionFunc={async () => {
                ...
                setLoading(true)
                try {
                  let res = await liquidate(transId, transOwner, parseInt(transDebt*1000000), parseInt(transPremium*1000000))
                  if (res.alert) {
