  ...
  for (let i = 0; i < metadata.length; i++) {
    try {
      const response = axios.get(
        "https://storage.googleapis.com/gard-dot-money/" + metadata[i]["image"],
      );
      final.push(metadata[i]);
      final[final.length - 1]["image"] =
        "https://storage.googleapis.com/gard-dot-money/" + metadata[i]["image"];
    } catch (ex) {
      console.log(ex);
    }
...
async function getArticleMetadata() {
  ...
  try {
    response = await axios.get(
      "https://storage.googleapis.com/gard-dot-money/article_info.json",
    );
  } catch (ex) {
    response = null;
    console.log(ex);
    ...
    <div style={{}}>
      <div style={{ paddingBottom: 40 }}>
        ...
        <div style={{ paddingBottom: 29 }}>
          <Subtitle>
            Welcome to the GARD protocol Web App! This is the MainNet
            application. Transactions made through this app are final and cannot
            be undone.
          </Subtitle>
        </div>
