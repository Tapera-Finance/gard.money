    ...
    let borrowDetails = mobile ? [
      {
        ...
        val: `$${mAsset == null || mAsset == "" ? ((1.20 * (mAlgosToAlgos(cdp.debt)) / (mAlgosToAlgos(cdp.collateral))).toFixed(4)) : ((1.20 * (mAlgosToAlgos(cdp.debt) + mAsset) / (mAlgosToAlgos(cdp.collateral))).toFixed(4))}`,
        hasToolTip: true,
      },
    ] : [
      {
